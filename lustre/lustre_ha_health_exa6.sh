...
sudo /usr/bin/env "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/opt/ddn/es/tools:/opt/ddn/scalers/tools:/root/bin" /usr/bin/hastatus -s | sed -n '/Failed\ Resource\ Actions/q;p' > "${tfile}"
...
sudo /usr/bin/env "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/opt/ddn/es/tools:/opt/ddn/scalers/tools:/root/bin" /usr/bin/hastatus -R | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" | sed -n '/Failed\ Resource\ Actions/q;p' > "${tfile}"
