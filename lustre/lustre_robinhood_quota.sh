...
do
	...
	while read -r line; do
        ...
        type_values=$(mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select type,sum(count) from ACCT_STAT where uid = '"${user_name}"' and projid = '"${p}"' group by type;" | tail -n +2 | sed 's/\t/=/' | xargs | sed 's/\ /,/g')
        echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},user_name=${user_name},report=inode_type ${type_values}"
	done <"${tfile}"
	...
	mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select gid,sum(size),sum(count) from ACCT_STAT where projid = '"${p}"' GROUP BY gid;" | tail -n +2 > "${tfile}"
        ...
        type_values=$(mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select type,sum(count) from ACCT_STAT where gid = '"${group_name}"' and projid = '"${p}"' group by type;" | tail -n +2 | sed 's/\t/=/' | xargs | sed 's/\ /,/g')
        echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},group_name=${group_name},report=inode_type ${type_values}"
        done <"${tfile}"
	...
	type_values=$(mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select type,sum(count) from ACCT_STAT where projid = '"${p}"' group by type;" | tail -n +2 | sed 's/\t/=/' | xargs | sed 's/\ /,/g')
	echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},report=inode_type ${type_values}"
done
...
type_values=$(mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select type,sum(count) from ACCT_STAT group by type;" | tail -n +2 | sed 's/\t/=/' | xargs | sed 's/\ /,/g')
echo "lustre_robinhood_quota,fs=${fs},report=inode_type ${type_values}"
