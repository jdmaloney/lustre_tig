...
do
	...
	while read -r line; do
		IFS=" " read -r user_name kb_used inodes <<< $(echo ${line})
		...
		echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},user_name=${user_name} kb_used=${kb_used},inodes=${inodes},sz0=${sz0},sz1=${sz1},sz32=${sz32},sz1K=${sz1K},sz32K=${sz32K},sz1M=${sz1M},sz32M=${sz32M},sz1G=${sz1G},sz32G=${sz32G},sz1T=${sz1T}"
	done <"${tfile}"

	## Gather per group stats in each project
	mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select uid,sum(size),sum(count) from ACCT_STAT where projid = '"${p}"' GROUP BY gid;" | tail -n +2 > "${tfile}"
        while read -r line; do
                IFS=" " read -r group_name kb_used inodes <<< $(echo ${line})
                ...
                echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},group_name=${group_name} kb_used=${kb_used},inodes=${inodes},sz0=${sz0},sz1=${sz1},sz32=${sz32},sz1K=${sz1K},sz32K=${sz32K},sz1M=${sz1M},sz32M=${sz32M},sz1G=${sz1G},sz32G=${sz32G},sz1T=${sz1T}"
        done <"${tfile}"
	...
	IFS=" " read -r sz0 sz1 sz32 sz1K sz32K sz1M sz32M sz1G sz32G sz1T <<< $(mysql -u ${robinhood_user} -p"${robinhood_password}" -D ${robinhood_db} -e "select sum(sz0),sum(sz1),sum(sz32),sum(sz1K),sum(sz32K),sum(sz1M),sum(sz32M),sum(sz1G),sum(sz32G),sum(sz1T) from ACCT_STAT where projid = '"${p}"' and type = 'file';" | tail -n +2 | sed 's/\t/\ /g' | sed 's/NULL/0/g')
                echo "lustre_robinhood_quota,fs=${fs},project_id=${p},project_name=${proj_name},type=full_project kb_used=${kb_used},inodes=${inodes},sz0=${sz0},sz1=${sz1},sz32=${sz32},sz1K=${sz1K},sz32K=${sz32K},sz1M=${sz1M},sz32M=${sz32M},sz1G=${sz1G},sz32G=${sz32G},sz1T=${sz1T}"
done
