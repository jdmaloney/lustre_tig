...
while IFS= read -r line; do
	...
	if [ -n "${optimal}" ] && [ "${name}" != "mgs" ] && [ ${mount_health} -eq 1 ]; then
		if [ "${optimal}" == "Mounted" ]; then
			echo "es_mount,disk_type=${disk_type},disk_name=${name} mount_state=\"${mount_state}\",mount_health=${mount_health},optimal_mount=1"
		else
			echo "es_mount,disk_type=${disk_type},disk_name=${name} mount_state=\"${mount_state}\",mount_health=${mount_health},optimal_mount=0"
		fi
